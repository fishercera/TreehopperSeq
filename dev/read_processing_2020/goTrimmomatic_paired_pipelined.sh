...
source ~/.profile
##### Set up variables
READDIR=/labs/Jockusch/cfisher/RAW_NovaSeqReads/
...
###From Manual
#ava -jar trimmomatic-0.30.jar PE s_1_1_sequence.txt.gz s_1_2_sequence.txt.gz lane1_forward_paired.fq.gz lane1_forward_unpaired.fq.gz lane1_reverse_paired.fq.gz lane1_reverse_unpaired.fq.gz ILLUMINACLIP:TruSeq3-PE.fa:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:36
#####
 ...
 then
     mv input/scratch/$CURRENT_LIB.P1.trimmed.fastq.gz output/$CURRENT_LIB.P1.trimmed.fastq.gz
     mv input/scratch/$CURRENT_LIB.P2.trimmed.fastq.gz output/$CURRENT_LIB.P2.trimmed.fastq.gz
     mv input/scratch/$CURRENT_LIB.U*.fastq.gz output/.
#     rm -f input/$LEFT
#     rm -f input/$RIGHT
