...
/labs/Wegrzyn/EnTAP/EnTAP --runP -d diamondDBs/uniref90.dmnd -d diamondDBs/sprot.dmnd -i Assemblies/$LIB.v2.Trinity.fasta  -a Alignments/$CURRENT_LIB\.$LIBEXT\.rsem.bowtie2.bam --taxon $SPECIES  -c bacteria -c fungi -c homo_sapiens -c viridiplantae -c archaea --threads 8 --out-dir $OUTDIR  --trim --ontology 0 --ontology 1 --protein pfam
wait
fi 
...
if [ -s $OUTDIR/final_annotations_lvl4_no_contam.tsv ]; 
    then
